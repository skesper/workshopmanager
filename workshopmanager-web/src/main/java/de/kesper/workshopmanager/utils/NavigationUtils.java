/*
 *  30.10.2014 (C) Stephan Kesper
 */
package de.kesper.workshopmanager.utils;

/**
 *
 * @author kesper
 */
public class NavigationUtils {

    public static final String CAR_EDIT = "/restricted/car/editcar";
    public static final String CAR_OVERVIEW = "/restricted/car/car";

    public static final String CUSTOMER_EDIT = "/restricted/customer/editcustomer";
    public static final String CUSTOMER_OVERVIEW = "/restricted/customer/customer";
    
    public static final String LOGIN = "/index";

    public static final String START = "/restricted/index";
}
